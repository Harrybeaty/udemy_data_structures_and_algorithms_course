...
class LinkedList:
    ...
    def remove(self, index):
        if index < 0 or index >= self.length:
            return None                 # return None because it is the opposite of returning a node.
        if index == 0:
            return self.pop_first()
        if index == self.length:
            return self.pop()
        prev = self.get(index - 1)      # Sets prev to point at node before the node we want to remove.
        temp = prev.next                # Sets temp to point to the node to remove.     
        prev.next = temp.next           # Sets Node that is before the one we wnat to remove to the node after.
        temp.next = None                # Set the removed node to point at None.
        self.length -= 1
        return temp
    
    def reverse(self):
        temp = self.head                # use temp to swap head and tail around.
        self.head = self.tail           
        self.tail = temp
        after = temp.next               # create after to go after temp
        before = None                   # before goes before temp, which is none because temp is at the front.
        for _ in range(self.length):
            after = temp.next           # after moves up one in the list.
            temp.next = before          # make node temp is pointing at reverse to pint at before node.
            before = temp               # move before up one.
            temp = after                # move temp up one.