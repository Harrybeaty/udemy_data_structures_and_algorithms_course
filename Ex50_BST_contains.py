""" Contains is used to find if a certain value is in a tree.
    Edge cases:
    tree empty self.root is None
    Return False

    base case:
    value matches node value
    value doesn't match node value
...
class BinarySearchTree:
    ...
    def contains(self, value):
        if self.root is None:       # You don't need these 2 lines of code.
            return False            # I kept them in because you need to think about the case.
        temp = self.root
        while temp is not None:
            if value > temp.value:
                temp = temp.right
